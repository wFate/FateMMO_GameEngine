...
![Lines of Code](https://img.shields.io/badge/LOC-202%2C500%2B-brightgreen?style=flat-square)
![Files](https://img.shields.io/badge/files-859-2ea44f?style=flat-square)
![Tests](https://img.shields.io/badge/tests-1%2C670_passing-brightgreen?style=flat-square)
![Protocol](https://img.shields.io/badge/protocol-v15-blueviolet?style=flat-square)
![Quests](https://img.shields.io/badge/quests-167-E040FB?style=flat-square)
![Migrations](https://img.shields.io/badge/SQL_migrations-119-FF7043?style=flat-square)
...
> 🧱 **202,500+ lines** across **859 files** — engine (91K), game (41K), server (38K), tests (32K)
...
### 🚧 v3 Incoming — Loot, Loadouts & Living Economy

> **Demo v3 is around the corner.** Five back-to-back content sessions just closed: a **full loot economy rebalance** (6 migrations rebanding the 7-tier enhance-stone ladder), the **drag-driven enchant flow** (protected stones are now real inventory items, no more ephemeral client-side staging), the **Merchant Pass + Recall Scroll** Opals SKUs, and the **polymorphic 20-slot skill loadout** that lets players drag potions onto their skill bar and fire them like spells. **Protocol v15** ships with three new wire packets and a renamed assignment opcode — fully backward-compatible test pass at 1,670 / 1,671 green.
>
...
> - ⚒️ **7-Tier Enhance-Stone Economy + Protected Stones as Physical Items (S134)** — `mat_enhance_stone_basic → legendary` rebanded in 10-level bands (1-10, 11-20, ... 61-70). Drag a Protection Scroll onto a base stone → server crafts a real `_protected` item that lives in inventory and survives logout. Drag-driven enchant flow now requires explicit `stoneSlot` from the wire — no more "dragged stone is cosmetic" gap. Risky-tier failure **destroys** equipment instead of flagging `isBroken`. Two confirm-dialog popups (`enchant_confirm_dialog`, `enchant_protect_confirm_dialog`) gate every attempt. Same-container constraint (inv↔inv or same-bag↔same-bag) on both enchant and craft drags.
> - 🎰 **Loot Rebalance Arc (S130–S133, migs 103-111)** — Average trash drops-per-hour cut from **12.27 → 1.26**; legendary/epic rates corrected on endgame elites; Crystal Caverns trash backfill (was starved); enhance-stone systemic +10 level offset bug fixed (every mob now drops the canonical band tier); apex stone supply pyramid (worldboss → boss → trash trickle). Final state: every band cleanly stocked; per-tier trash rate ladder from 0.002 (basic) → 0.0005 (ancient) → 0.0001 (apex trickle). Numbers locked in `project_loot_rebalance_targets.md`.
> - 💎 **Merchant Pass + Recall Scroll Wired (S132)** — `item_merchant_pass` (5000 opals, 30 days marketplace listing access, **stacking from current expiry**) gates the Marketplace `ListItem` flow. `recall_scroll` purchasable for 500 opals OR 500 gold. New `max_marketplace_listings` per-character column (defaults 7) so future "Listing Slot Extender" SKUs can `+= grant`. Character-select **PREMIUM pill**, market-panel **`PASS Nd` / `NO PASS` status pill**, and premium-item tooltip descriptions populated client-side.
> - ✨ **Chrome System Default-On (S125–S127)** — 25 per-panel + 3 manager `xxxUseChrome_` toggles flipped to `true`. 14 chrome styles (panel_chrome / title_pill / close_x + 8 panel variants + tooltip + tab_rail). View-menu items kept as legacy rollback. Inter SemiBold across the board; parchment, HUD-dark, dialog, scrollbar, tab, and tooltip themes all reconciled into one polished aesthetic.
> - 🌳 **Phase 3C Ambient Mini-Chains (S119)** — 30 new side quests (Q730-Q822) across 10 NPC chains, ~6,500 brutal kills, themed stat scrolls + Fate's Grace + tier-matched enhance stones (basic→ancient + protected variants on capstones) on every Q3.
...
| 🏗️ | **202,500+ LOC** of hand-written C++23 across engine, game, server, and tests — no code generation, no middleware, no AAA framework license fees |
|:---:|:---|
...
| 🛡️ | **Hardened v15 protocol** — 64-bit CSPRNG session tokens, timing-safe auth comparisons, anti-replay nonces, 64-bit ACK window, `CmdAckExtended` recovery, DB-backed `auth_sessions` with cross-process `LISTEN/NOTIFY` kick |
...
| 🪡 | **Polymorphic Skill-Arc Loadout (S135)** — drag potions/recall scrolls onto any of 20 slots (5 × 4 pages), bindings reference `instance_id`, persist across logout, per-item cooldowns, **14 stale-binding sweep sites** keep everything coherent across trade/sell/destroy/craft/bank flows |
...
| ⚔️ | **Phase 3C ambient grind (S119)** — 30 new side quests (Q730-Q822) across 10 NPC chains, ~6,500 brutal kills, themed stat scrolls + Fate's Grace + tier-matched enhance stones on every Q3 capstone |
| 🎰 | **Locked loot economy** — 6-mig rebalance arc collapsed avg trash drops/hour from 12.27 → 1.26, fixed systemic +10 enhance-stone offset, apex stone pyramid (worldboss 0.03 → boss 0.005 → trash 0.0001 trickle), per-tier ladder targets in `project_loot_rebalance_targets.md` |
| 💎 | **Merchant Pass + Listing Slot extensibility** — 5000-opal pass stacks expiry from current end-date, gates marketplace List flow, per-character `max_marketplace_listings` column ready for future Slot Extender SKUs |
| 🗄️ | **DB-driven content engine** — **119 numbered SQL migrations**, **816 hand-tuned items** across 6 canonical types, **17 PostgreSQL repository files** + **12 startup caches** with fiber-based async dispatch (zero game-thread blocking) |
...
| 🧪 | **1,670 automated tests** across 220 test files keeping every subsystem honest (10,909 assertions, all green except one pre-existing tier-bracket fixture) |
...
| 📡 | **Protocol v15 custom UDP** — critical-lane bypass for 9 load-bearing opcodes, `SvEntityEnterBatch` coalesce, 32-bit delta compression, fiber-based async DB dispatch, client-side `CharacterFlags` mirror, `SvConsumableCooldown` poke (0xE9), `CmdCraftProtectStone` (0xEA) for protected-stone crafting |
...
- **🆕 Skill-Arc Consumable Bindings (S135)** — Drag HP/MP potions, recall scrolls, fated potions, or food onto any of the 20 loadout slots from inventory grid OR open bag overlay. Tap on the in-combat HUD arc consumes one stack and triggers a per-item-type cooldown (5s combat / 30s premium). Bindings reference stable `instance_id`s, persist across logout via the polymorphic `character_skill_bar` schema, and auto-clear when the bound stack is destroyed via the **14-site stale-binding sweep**. New `SkillLoadout` UI-facing facade owns the canonical state; `SkillArc` HUD widget is now a thin renderer over the model. New `SkillLoadoutStrip` widget embedded in the inventory panel for drag/drop loadout management.
...
- **🆕 Phase 3C Ambient Mini-Chains (10 chains × 3 quests = 30 side quests)** — Bram, Hadrik, Tessen, Mira+Vell, Sister Sand+Old Cinder, Toren, Vael, Vess, Ash, Mora promoted to quest-givers. ~6,500 brutal kills across all 30 quests; capstones reach 650 kills (Q792 Vael at 4 mob types). Honor 3-5× the formula on capstones (200-800 explicit override). Currency-only rewards using existing items: gold + EXP + opals + 10 themed stat scrolls + Fate's Grace + tier-matched enhance stones (basic→ancient + protected variants on capstones).
...
- **65+ Widget Types:** 25 Engine-Generic (Panels, ScrollViews, ProgressBars, Checkboxes, ConfirmDialogs with serializable bgColor/borderColor/messageOffset, NotificationToasts, LoginScreen, ImageBox, TextInput with masked password mode, **PanelChrome / TabRail / DebugChromePanel / Divider** chrome primitives) and **40+ Game-Specific** (DPad, **SkillArc** with 4-page polymorphic C-arc, **🆕 SkillLoadoutStrip** for in-panel drag/drop loadout management, FateStatusBar, InventoryPanel with paper doll, CostumePanel, CollectionPanel, ArenaPanel, BattlefieldPanel, **PetPanel**, **OpalsShopPanel**, **BountyPanel**, **FriendsPanel**, **GauntletHUD**, **GauntletResultModal**, CraftingPanel, MarketPanel with buy confirmation + status lifecycle + **PASS pill**, BagViewPanel, EmoticonPanel, QuantitySelector, PlayerContextMenu, ChatIdleOverlay, BossHPBar, **NpcDialoguePanel** with state-aware quest-tree picker, DeathOverlay with Fate's Grace button, **enchant_confirm_dialog**, **enchant_protect_confirm_dialog**, and more).
- **10 JSON Screens & 42 Theme Styles:** Parchment, HUD dark, dialog, tab, scrollbar, **14 chrome styles** (panel_chrome.default / title_pill / close_x + 8 panel variants + tooltip; tab_rail.default / active / inactive). **Chrome default-on as of S127** — 25 per-panel + 3 manager `xxxUseChrome_` toggles flipped to `true`. View-menu items kept as legacy rollback. Full serialization of layout properties, fonts, colors, and inline style overrides. Ctrl+S dual-save (build + source dir) via atomic write. Hot-reload with 0.5s polling + suppress-after-save guard.
...
- **Mobile-First, No Right-Click Design:** Every interaction works on touch — long-press, drag-and-drop, tap, confirm popups. The polymorphic skill-arc loadout, the enchant flow, and the protect-craft flow are all drag-only — no context menus required.
- **120+ UI tests.**
...
**Headless 20 Hz server** (`FateServer`) with max **2,000 concurrent connections**. **47 handler files**, **17 DB repository files**, **12 startup caches**, **15-min idle timeout**, graceful shutdown with player save flush. Every game action is server-validated — zero trust client. **PROTOCOL_VERSION 15** — full v9 reliability rebuild + v10 interact-site framework + v11 site-string-id swap + v12 `CharacterFlags` client mirror + v13 `SvConsumableCooldown` + v14 explicit-stone enchant + v15 `CmdCraftProtectStone` for protected-stone crafting.
...
| Property | Value |
|----------|-------|
| Protocol | Custom reliable UDP (`0xFA7E`, **v15**), Win32 + POSIX |
...
| Packets | 26-byte header (v9+: ackBits widened 32→64-bit), RTT estimation (EWMA 0.875/0.125), retransmission delay `max(0.2s, 2*RTT)`, zero-copy retransmit. Pending-packet queue 2048 slots, 75% congestion threshold. **`SvEntityEnterBatch`** coalesce, **`CmdAckExtended`** out-of-window recovery, epoch-gated `Rekey`. **v13–v15 packets:** `SvConsumableCooldown` (0xE9), `CmdCraftProtectStone` (0xEA), `SvCraftProtectStoneResult` (0xEB) |
...
- **Network Panel** — Editor dock surfacing client-side metrics: protocol banner (`v15`), encryption status, RTT (color-coded), reliable queue depth (color-coded against 2048 cap), dropped-non-critical count, AOI entity count, host:port, Connect/Disconnect button.
...
| **PROTOCOL v9 → v15 in 6 sessions** | v9: 64-bit ACK window, `SvEntityEnterBatch` coalesce, `CmdAckExtended`, epoch-gated rekey. v10: interact-site packets. v11: site-string-id swap. v12: `CharacterFlags` client mirror. v13: `SvConsumableCooldown` (consumable cooldown poke after use). v14: explicit-stone enchant on the wire. v15: `CmdCraftProtectStone` + `SvCraftProtectStoneResult` for protected-stone crafting |
...
| **fate_tests** | 1,670 unit tests (doctest, 10,909 assertions) | When `tests/` sources present |
...
The engine maintains exceptional stability through **1,670 passing test cases** across **220 test files**, powered by `doctest`. **10,909 assertions** with one pre-existing tier-bracket fixture failure (independent of any active arc).
...
```
engine/                    # 91,486 LOC — Core engine (20 subsystems, 378 files)
 render/                   #   Sprite batching, SDF text, lighting, bloom, paper doll, VFX, Metal RHI
 net/                      #   Custom UDP (v15), Noise_NK crypto, AuthProof, replication, AOI, interpolation
 ecs/                      #   Archetype ECS (4096 reserve), 56 components, reflection, serialization
 ui/                       #   65+ widgets (incl. SkillLoadoutStrip, ConfirmDialog chrome, FriendsPanel, BountyPanel), JSON screens, themes, viewport scaling, panel chrome
 editor/                   #   ImGui editor, undo/redo, Aseprite animation editor, asset browser, Play/Observe/Scene toolbar, 28-field strip inspector
...
game/                      # 41,023 LOC — Game logic layer (117 files)
 shared/                   #   19,307 LOC of pure gameplay across 75 files (zero engine deps)
   ...
   enchant_system          #   7-tier ladder (basic→legendary), protected stones as physical items, drag-driven flow
...
server/                    # 38,223 LOC — Headless authoritative server (142 files)
 handlers/                 #   47 packet handler files (incl. loadout_sweep, interact_site, crafting w/ enchant + protect-craft)
...
tests/                     # 31,794 LOC — 1,670 passing test cases across 220 files (10,909 assertions)

ads_ssv_server/            # Standalone rewarded-video verifier (ECDSA-signed HTTPS callbacks)
Docs/migrations/           # 119 numbered SQL migrations (schema + content seeds)
...
<img src="https://img.shields.io/badge/C%2B%2B-202%2C500%2B_lines-00599C?style=flat-square&logo=cplusplus&logoColor=white" alt="C++ LOC" />
<img src="https://img.shields.io/badge/files-859-2ea44f?style=flat-square" alt="Files" />
<img src="https://img.shields.io/badge/tests-1%2C670_passing-2ea44f?style=flat-square" alt="Tests" />
<img src="https://img.shields.io/badge/assertions-10%2C909-success?style=flat-square" alt="Assertions" />
<img src="https://img.shields.io/badge/protocol-v15-blueviolet?style=flat-square" alt="Protocol" />
...
<img src="https://img.shields.io/badge/migrations-119-FF7043?style=flat-square" alt="Migrations" />
