...
![Lines of Code](https://img.shields.io/badge/LOC-133%2C900%2B-brightgreen?style=flat-square)
![Tests](https://img.shields.io/badge/tests-1%2C203-brightgreen?style=flat-square)
...
> **133,900+ lines** across **696 files** — engine (60K), game (30.1K), server (22.9K), tests (20.8K)
...
|----------|-----------|
| **Language** | Modern C++23 (MSVC, GCC 13, Clang 17). `std::expected`, structured bindings, fold expressions throughout |
...
| **ECS** | Data-oriented archetype ECS, contiguous SoA memory, **53 registered components**, generational handles, prefab variants (JSON Patch), compile-time `CompId`, Hot/Warm/Cold tier classification, `FATE_REFLECT` macro with field-level metadata |
...
- **Optimistic Combat** — TWOM-style attack windups play immediately to hide latency; 3-frame windup (300ms), hit frame with predicted damage text + procedural lunge offset. `CombatPredictionBuffer` ring buffer (32 slots). Server reconciles final damage.
...
- **Skill VFX Pipeline** — TWOM-style composable effects: JSON definitions with 4 optional phases (Cast/Projectile/Impact/Area). Sprite sheet animations + particle embellishments. 32 max active effects. 13 tests.
...
- **Mob AI** — TWOM cardinal-only movement, L-shaped chase, axis locking, wiggle unstuck, roam/idle phases, threat-based aggro tables, `shouldBlockDamage` callback (god mode). Wall-stuck is intentional (TWOM lure-and-farm mechanic).
...
- **56 Widget Types:** 21 Engine-Generic (Panels, ScrollViews, ProgressBars, Checkboxes, ConfirmDialogs, NotificationToasts, LoginScreen) and **29 Game-Specific** (DPad, SkillArc with 4-page C-arc, FateStatusBar, InventoryPanel with paper doll, CostumePanel, CollectionPanel, and 23 more) + 6 internal layout primitives.
- **12 JSON Screens & 28 Theme Styles:** Parchment, HUD dark, dialog, tab, scrollbar themes. Full serialization of layout properties, fonts, colors, and inline style overrides. Ctrl+S dual-save (build + source dir).
...
- **ImGui Fully Retired:** Zero ImGui rendering in game client. All HUD/nameplates/floating text render via SDFText + SpriteBatch. ImGui stripped from shipping builds, editor build unchanged.
- **95+ UI tests.**
...
**Headless 20 Hz server** (`FateServer`) with max 2,000 concurrent connections. **37 handler files**, **16 DB repositories**, **10 startup caches**. Every game action is server-validated.
...
<details>
<summary><b>GM Command System</b></summary>
...
- **Live Inspector** — Edit all 53 component types live with **full undo/redo**. Sprite preview thumbnails. Reflection-driven generic fallback via `FATE_REFLECT`.
...
- **Animation Editor** — TWOM-style: **Sprite Sheet Slicer** (grid overlay, cell assignment, quick templates, `.meta.json` output) + legacy Individual Frame mode. Hit frame editor, preview with play/pause/step, 3-direction authoring → 4-direction runtime.
...
|----------|--------|---------|
| **Windows** | Developer Build | MSVC, primary development target |
...
| **Linux**   | CI Builds | GCC 13, Clang 17, target environment for headless server deployment |

---

## 🧪 Testing

The engine maintains exceptional stability through **1,203 test cases** across **168 test files**, powered by `doctest`.

```bash
# Run all 1,203 unit & scenario tests (headless, no server required):
...
out/build/x64-Debug/fate_tests.exe -tc="SkillVFX*"
out/build/x64-Debug/fate_tests.exe -tc="CollisionGrid*"
```
...
```
engine/                    # 60,070 LOC — Core engine (20 subsystems)
...
├── ui/                    #   56 widgets, JSON screens, themes, viewport scaling
...
game/                      # 30,140 LOC — Game logic layer
...
│   ├── mob_ai             #   TWOM cardinal movement, threat, leash, L-shaped chase
...
server/                    # 22,912 LOC — Headless authoritative server
├── handlers/              #   37 packet handler files (split from monolith)
...
tests/                     # 20,770 LOC — 1,203 test cases across 168 files
```
...
<img src="https://img.shields.io/badge/C%2B%2B-133%2C900%2B_lines-00599C?style=flat-square&logo=cplusplus&logoColor=white" alt="C++ LOC" />
<img src="https://img.shields.io/badge/files-696-2ea44f?style=flat-square" alt="Files" />
<img src="https://img.shields.io/badge/tests-1%2C203_passing-2ea44f?style=flat-square" alt="Tests" />
...
<img src="https://img.shields.io/badge/UI_widgets-56-ff69b4?style=flat-square" alt="UI Widgets" />
