...
![Lines of Code](https://img.shields.io/badge/LOC-184%2C000%2B-brightgreen?style=flat-square)
![Files](https://img.shields.io/badge/files-867%2B-2ea44f?style=flat-square)
...
> 🧱 **184,000+ lines** across **867+ files** — engine (82K), game (37K), server (35K), tests (28K), shaders (~700)
...
### 🚧 v3 Incoming — Loot, Loadouts, Chrome, Drain Arc & Living Economy

> **Demo v3 is around the corner.** A long arc of back-to-back work just closed: a **full loot economy rebalance** (6 migrations rebanding the 7-tier enhance-stone ladder), the **drag-driven enchant flow** (protected stones are now real inventory items, no more ephemeral client-side staging), the **Merchant Pass + Recall Scroll** Opals SKUs, the **polymorphic 20-slot skill loadout** that lets players drag potions onto their skill bar and fire them like spells, **chrome'd HUD bespokes** (DPad / SkillArc / SettingsPanel / MenuTabBar — 49 styled fields), the **bag-item NPC sell pipeline** (sell straight from bag sub-slots, no retrieve dance), a **drag-bind pipeline repair** that finally wires the SkillLoadoutStrip end-to-end, a **server Drain Arc** (clean shutdown wall time **70s → 2s**, JSONB-batched inventory UPSERT **12s → 0.5s**), a real **multi-channel SDF text pipeline** with offline-baked Fredoka MTSDF replacing 9-pass fake outlines, a **pet lifecycle cache+replay** so equipped pets persist visibly across every zone transition, **single-instance UDP port exclusivity** so duplicate `FateServer` instances refuse to bind silently, and a **replicated-entity save filter** so server-spawned entities can never bleed into authored scene JSON. **Protocol v20** ships with eight new wire packets, two batched-update opcodes, a polymorphic assignment opcode, and a system-packet envelope-compat bump. Tests at **1,687 / 1,688 green**.
>
> 🎯 **Headline drops (since v2):**
> - ⚡ **Drain Arc — Server Shutdown Hardening + JSONB Inventory Batch (protocol v20)** — Pre-arc Ctrl-C took **70.6 seconds** with 14 DB jobs orphaned, the synchronous final scene save skipped, and disconnect-final saves missing entirely. Six sub-arcs (D.1–D.5.1, E.1) shipped as one continuous push: per-character **`AsyncSaveState` in-flight gate** (31 stacked `player.saveAsyncAtomic` jobs for one player → max 1 in-flight + coalesced follow-up), audited and migrated 8 direct `savePlayerToDBAsync` callsites onto the gate, **lock-wait + section-timing instrumentation** that surfaced inventory as the 12s-of-12.5s long pole, **JSONB-batched `character_inventory` UPSERT** (89 per-row exec_params → single `jsonb_to_recordset` round-trip; ~6.2s → ~500ms), and a **scene mob-death batch** (per-row INSERT loop in 3 sites → batched, 17–37s → 0.57s for 181 mobs). Final wall time: **70.6s → 2.06s.** No UNSAFE shutdown logs, no skipped final saves. `PROTOCOL_VERSION` bumped to **v20** (`CmdAckExtended` envelope-compatibility — old/new mixes refuse handshake instead of silently regressing the stranded-ACK failure).
> - 🔤 **Single-Pass Text-Effects Pipeline + Real MTSDF Atlas (Fredoka SemiBold)** — Nameplates were faking outline by stamping each string 9–25 times in 8-directional pixel offsets at `TextStyle::Normal`. Replaced with a single shader-side composite (`drawWorldEffects`) — outline + shadow + fill in one pass, then routed `fate_round_ui` through a real `msdf-atlas-gen v1.4` MTSDF bake (177 glyphs, 512×512, 4 px range) so outline expansion math operates on actual signed distance data instead of bitmap coverage. Static `Fredoka-SemiBold.ttf` instanced offline via fontTools (Google's repo only ships the variable font). New `applyDefault` parameter separates two intents — legacy fake-outline callers get the chunky shader-side default; config-driven callers (Admin role tag's `outlineThickness:0`) render exactly as configured. **Pixel→SDF unit conversion fix** so editor-authored 0..8 outline thickness no longer saturates at max regardless of value.
> - 🐾 **Pet Lifecycle Cache + Replay on Zone Transition** — Equipped pet sprite vanished after every zone transition and never returned until the next discrete pet event (equip/unequip/login). Two independent dropouts in the `LoadingScene → InGame → local-player-create` window were silently discarding `SvPetState`. Fixed with a cache-and-replay around the packet (mirroring the `pendingPlayerState_` / `pendingRespawn_` precedents) — every server-pushed snapshot now persists across the deferred-creation window and replays the moment the local player is created in the new scene. Triple-pass live trace verified across login → village A → village B → village A: pet visibly follows in every scene.
> - 🛡️ **Replicated-Entity Save Filter + Pet Bake Repair** — Editor saves used to risk bleeding server-spawned entities into authored scene JSON. New runtime flag `Entity::setReplicated(true)` is set by every runtime factory (mob/boss/ghost/dropped_item/NPC ghost); editor scene-save skips entities flagged as replicated. Inspector badges (`[REPLICATED]` / `[AUTHORED]`) make the state visible. Promote-on-edit clears the flag on real content delta so authored edits persist. Closes a long-standing class of "phantom NPCs in authored scene" hazards.
> - 🌐 **Single-Instance UDP Port Exclusivity (Windows)** — Two `FateServer` processes could co-bind UDP 7777 because IPv6-dual-stack + IPv4-only on the same port is allowed under default Windows semantics; datagrams between the two co-bound sockets split unpredictably (most plausible cause of an earlier mystery DC where heartbeats started landing on a different server than the one a client had auth-handshaken with). Fix: `SO_EXCLUSIVEADDRUSE` set before `bind()` on both IPv6 dual-stack and IPv4 paths; on `WSAEADDRINUSE` for IPv6 we no longer fall through to IPv4. Duplicate launches now fail loudly with `[Net] bind() failed: 10048`.
> - 📦 **Critical-Lane Batch Opcodes + Per-Opcode Net Telemetry (protocol v18 / v19)** — `SvSkillResultBatch` (0xEC, v18) coalesces multi-target AOE skill results into one batch packet so an Inferno landing on 30 mobs no longer spams 30 individual reliables and overruns the pending queue under crowd density. `SvEntityLeaveBatch` (0xED, v19) does the same for AOI deactivation bursts. Both are critical-lane (bypass congestion check). Phase-A `NetServer::OpcodeStats` per-opcode counters dump on every 5s tick-stats line and on congestion-fire — root-cause visibility for any future packet-flood incident.
> - 🧰 **Drop-Routing Fall-Through + Strip Cross-Rect Hit Test** — UIManager's drop walk now mirrors `handlePress`, walking the screen stack instead of stopping at the first non-interactive root — fixed `npc_root` `StretchAll` swallowing drops onto strips behind it. `InventoryPanel::hitTest` override reaches strip slots that paint outside `computedRect_` via origin offset.
...
| 🏗️ | **184,000+ LOC** of hand-written C++23 across engine, game, server, tests, and shaders — no code generation, no middleware, no AAA framework license fees |
|:---:|:---|
...
| 🛡️ | **Hardened v20 protocol** — 64-bit CSPRNG session tokens, timing-safe auth comparisons, anti-replay nonces, 64-bit ACK window, `CmdAckExtended` recovery (envelope-compat-bumped to v20 to guard old/new mixes), DB-backed `auth_sessions` with cross-process `LISTEN/NOTIFY` kick |
| ⚡ | **Drain Arc — clean shutdown 70s → 2s** — Per-character async-save in-flight gate, JSONB-batched inventory UPSERT (12s → 0.5s), batched scene mob-death persistence (17–37s → 0.57s for 181 mobs). Six sub-arcs, one continuous push, zero protocol-breaking changes |
| 🔤 | **Real multi-channel SDF text** — Offline `msdf-atlas-gen` MTSDF bake (177 glyphs / 512×512 / 4 px range) with single-pass shader-side outline + shadow compositing. Replaces 9-pass fake-outline pixel stamping. Fredoka SemiBold + Inter SemiBold + PressStart2P + PixelifySans, with editor-authored pixel→SDF unit conversion |
| 🐾 | **Pet lifecycle cache + replay** — Cached `SvPetState` snapshot replays at the moment of local-player creation in every new scene, closing a window where two independent timing dropouts silently discarded the packet. Pets visibly follow across every zone transition |
| 🛡️ | **Replicated-entity save filter** — Runtime `Entity::setReplicated(true)` flag on every server-spawned entity; editor scene-save skips them. Inspector badges (`[REPLICATED]`/`[AUTHORED]`) + promote-on-edit on real content delta. Closes a class of "phantom NPCs in authored scene" hazards |
| 🌐 | **Single-instance UDP port exclusivity (Windows)** — `SO_EXCLUSIVEADDRUSE` on both IPv6 dual-stack and IPv4 paths; duplicate `FateServer` instances now refuse to bind instead of co-binding and silently splitting datagrams between processes |
| 📦 | **Critical-lane batched updates (v18 / v19)** — `SvSkillResultBatch` (0xEC) coalesces multi-target AOE results; `SvEntityLeaveBatch` (0xED) coalesces AOI deactivation bursts. Both bypass congestion checks. Plus per-opcode `NetServer::OpcodeStats` for instant root-cause visibility on packet floods |
...
| 🪡 | **Polymorphic Skill-Arc Loadout** — drag potions / recall scrolls onto any of 20 slots (5 × 4 pages) from main inventory OR inside an open bag, bindings reference `instance_id`, persist across logout, per-item cooldowns, **14 stale-binding sweep sites** keep everything coherent across trade/sell/destroy/craft/bank flows. Drop-pipeline ordering bug fixed + circle slots + cursor-tracking drop-highlight |
| 🛒 | **Bag-Item NPC Sell — `CmdShopSellFromBag = 0x50` (protocol v17)** — first-class wire packet for selling items that live in bag sub-slots; server emits both inventory + bag-content sync post-credit so the bag mirror never goes stale |
...
| ✨ | **Bespoke Widget Chrome — Checkpoint 8** — `MenuTabBar` / `SettingsPanel` / `DPad` / `SkillArc` joined the chrome system through 49 new styled fields routed via existing `panelUseChrome_` / `hudUseChrome_` toggles. Legacy `drawMetallicCircle` paths preserved verbatim under `else` for instant rollback |
...
| ⚔️ | **Phase 3C ambient grind** — 30 new side quests across 10 NPC chains, themed stat scrolls + Fate's Grace + tier-matched enhance stones on every capstone |
...
| 📡 | **Protocol v20 custom UDP** — critical-lane bypass for 11 load-bearing opcodes, `SvEntityEnterBatch` / `SvSkillResultBatch` / `SvEntityLeaveBatch` coalesce, 32-bit delta compression, fiber-based async DB dispatch, client-side `CharacterFlags` mirror, `SvConsumableCooldown` (0xE9), `CmdCraftProtectStone` (0xEA), `CmdShopSellFromBag` (0x50), `SvSkillResultBatch` (0xEC), `SvEntityLeaveBatch` (0xED) |
...
- **🆕 Skill-Arc Consumable Bindings + Drag-Bind Repair** — Drag HP/MP potions, recall scrolls, fated potions, or food onto any of the 20 loadout slots from the inventory grid OR an open bag overlay. Tap on the in-combat HUD arc consumes one stack and triggers a per-item-type cooldown (5s combat / 30s premium). Bindings reference stable `instance_id`s, persist across logout via the polymorphic `character_skill_bar` schema, and auto-clear when the bound stack is destroyed via the **14-site stale-binding sweep**. `SkillLoadout` UI-facing facade owns canonical state; `SkillArc` HUD widget is a thin renderer over the model. The `SkillLoadoutStrip` widget embedded in the inventory panel had its drop pipeline rebuilt — `UIManager::handleRelease` now sets `dropLocalPos` *before* `acceptsDrop`, `InventoryPanel::onRelease` no longer clobbers the shared `DragPayload` for strip drops, page dots are clickable, slots render as true circles, and the drop-highlight ring follows the cursor across slots.
...
- **10 JSON Screens & 42 Theme Styles:** Parchment, HUD dark, dialog, tab, scrollbar, **14 chrome styles** (panel_chrome.default / title_pill / close_x + 8 panel variants + tooltip; tab_rail.default / active / inactive). **Chrome default-on** — 25 per-panel + 3 manager `xxxUseChrome_` toggles set to `true`; **extended to 4 bespoke-shape widgets via 49 new styled fields** (no new manager toggles, no new View-menu items — propagation walk in `engine/ui/ui_manager.cpp:350-362` extends to `MenuTabBar` / `SettingsPanel` / `DPad` / `SkillArc`). Legacy `drawMetallicCircle` paths preserved verbatim under `else` for instant rollback. Full serialization of layout properties, fonts, colors, and inline style overrides. Ctrl+S dual-save (build + source dir) via atomic write. Hot-reload with 0.5s polling + suppress-after-save guard.
...
**Headless 20 Hz server** (`FateServer`) with max **2,000 concurrent connections**. **47 handler files**, **17 DB repository files**, **12 startup caches**, **15-min idle timeout**, graceful shutdown with player save flush — wall time **70s → 2s** post-Drain-Arc. Every game action is server-validated — zero trust client. **PROTOCOL_VERSION 20** — v9 reliability rebuild + v10 interact-site framework + v11 site-string-id swap + v12 `CharacterFlags` client mirror + v13 `SvConsumableCooldown` + v14 explicit-stone enchant + v15 `CmdCraftProtectStone` + v16 polymorphic skill-bar bindings (`CmdAssignSlot` rename + `kind`/`instanceId` extension) + v17 `CmdShopSellFromBag` (0x50) for bag-source NPC sells + v18 `SvSkillResultBatch` (0xEC) for AOE coalesce + v19 `SvEntityLeaveBatch` (0xED) for AOI deactivation coalesce + v20 `CmdAckExtended` envelope-compatibility (system-packet whitelist tightened so old/new client-server mixes refuse handshake instead of silently regressing the stranded-ACK failure).
...
| Critical-Lane Bypass | **11 load-bearing opcodes** (`SvEntityEnter`, `SvEntityLeave`, `SvPlayerState`, `SvZoneTransition`, `SvDeathNotify`, `SvRespawn`, `SvKick`, `SvScenePopulated`, `SvEntityEnterBatch`, `SvSkillResultBatch`, `SvEntityLeaveBatch`) bypass reliable-queue congestion check — eliminates "invisible mob", "death overlay never fires", and AOE skill-result drops under load |
...
- **🆕 Premium State Replication** — `CharacterPreview` (login char-select) + `SvPlayerStateMsg` (in-game live state) extended with `merchantPassExpiresAtUnix` + `maxMarketplaceListings`. Append-only — no protocol bump needed (lockstep deploy).
...
- **Live Inspector** — Edit all 56 component types live with **full undo/redo**. Sprite preview thumbnails. Reflection-driven generic fallback via `FATE_REFLECT`. SemiBold headings, separator lines. **🆕 Skill Loadout Strip + Strip Chrome inspector blocks** — 28 Inspector entries covering visibility, layout, slot/page selector, useChrome toggle, all 14 chrome color/thickness/font fields. **🆕 Bespoke Widget Chrome inspectors** — 49 new fields across 4 widgets (`MenuTabBar` 11, `SettingsPanel` 16, `DPad` 9, `SkillArc` 13), each routed through `checkUndoCapture(uiMgr)` for undo parity. Every entry undo-captured.
...
- **Network Panel** — Editor dock surfacing client-side metrics: protocol banner (`v20`), encryption status, RTT (color-coded), reliable queue depth (color-coded against 2048 cap), dropped-non-critical count, AOI entity count, host:port, Connect/Disconnect button.
...
| 🆕 **Real MTSDF Atlas + Single-Pass Text Effects** | Nameplates were faking outline by drawing each string 9–25 times in 8-directional pixel offsets. Replaced with single shader-side composite (outline + shadow + fill in one pass) plus a real `msdf-atlas-gen v1.4` Fredoka SemiBold MTSDF bake (177 glyphs, 512×512, 4 px range). Outline expansion math now operates on actual signed distance data instead of bitmap coverage. Editor-authored 0..8 outline thickness pixel→SDF unit conversion fix so values no longer saturate at max |
...
| 🆕 **Chrome Regression Sweep** | Six discrete close-X / chrome interaction bugs swept together: CollectionPanel's close-X recursed into stack-overflow (client locked up, server timed out the silent connection); SettingsPanel + GuildPanel (menu-tab copy) + CostumePanel close-Xs were silently no-ops with `onClose` never wired; ShopPanel chrome dropped sell-confirm clicks because `onPressChrome` had `if (showSellConfirm_) return true;` swallowing the popup; the inventory close-X didn't tear down the surrounding shop UX. Plus SkillArc Action no longer bleeds the legacy sword icon through chrome's flat fill, and AA-edge ring on Attack/PickUp no longer reads as a black border when chrome border matches the fill |
| 🆕 **Bespoke Widget Chrome — Checkpoint 8** | `MenuTabBar` / `SettingsPanel` / `DPad` / `SkillArc` joined the chrome system through 49 new styled fields (11+16+9+13) routed via existing `panelUseChrome_` / `hudUseChrome_` propagation walks at `engine/ui/ui_manager.cpp:350-362`. Chrome path replaces `drawCircle` / `drawRing` / `drawMetallicCircle` with `RoundedRectParams` equivalents; legacy `drawMetallicCircle` paths preserved verbatim under `else` (3 occurrences in SkillArc) for instant rollback. Defaults mirror legacy colors byte-for-byte. **No new manager toggles, no new View-menu items** — extends existing infrastructure. Client-only |
| 🆕 **Inventory Bag Panel — Geometry Fix + `bagPanelOffset` Knob** | Three nested issues unwound. (A) Brown panel was *centered* in the grid area while slots inside were *top-anchored*, so layouts drifted with bag size — fix anchors panel at top of grid area, slots/title/hit-test all derive from `bagRect.x/y + panelPad + …` regardless of size. (B) Local-vs-absolute coordinate space mismatch between `getBagPanelRect()` (local) and `renderBagView` (absolute) created a "can't click top slots" regression — fix translates local to absolute consistently in render, hit-test stays local. (C) New `Vec2 bagPanelOffset` inspector knob for fine-tuning. Client-only |
| 🆕 **Polymorphic 20-slot skill loadout** | Drag potions/recall scrolls/fated potions/food onto any of the 20 loadout slots from inventory or open bag. New `SkillLoadout` model owns canonical state; `SkillArc` HUD refactored to thin renderer; new `SkillLoadoutStrip` widget embedded in `InventoryPanel`. **14 stale-binding sweep sites** (trade/sell/market/destroy/bank/consume/craft/bag/revive/teleport/inventory-merge/soul-anchor) keep bindings coherent. Two migrations (`item_definitions.cooldown_ms` + `character_skill_bar` polymorphic columns). 56 new tests across `test_skill_loadout` + `test_skill_loadout_strip` + `test_skill_bar_polymorphic` |
| 🆕 **7-tier enhance-stone economy + protected stones as physical items** | Stone tier ladder consolidated 8→7, epic re-banded as Lv 51-60, legendary re-banded as Lv 61-70. **Drag-driven enchant flow** carries explicit `stoneSlot` on the wire (no first-match-by-id fallback). **Protected stones become real `_protected` items** that live in inventory and persist across logout — atomic post-consume-aware space check + rollback path. Two confirm-dialog popups gate every attempt. Risky-tier failure now destroys equipment (no more `isBroken` flag). Protocol 13→14→15 |
| 🆕 **Locked loot economy** | Avg trash drops/hour collapsed from **12.27 → 1.26** (Pass A). Endgame elite legendary/epic rates corrected on TheVoid + SkySanctuary mobs. Crystal Caverns trash backfilled (was starved). **Enhance-stone systemic +10 level offset bug fixed** — every mob now drops the canonical 10-level band tier. Apex stone supply pyramid (worldboss 0.03 → boss 0.005 → trash 0.0001 trickle). Per-tier trash rate ladder: basic 0.002 / refined 0.0015 / superior 0.001 / greater 0.001 / master 0.0007 / ancient 0.0005 |
| 🆕 **Merchant Pass + Recall Scroll wired end-to-end** | `item_merchant_pass` (5000 opals, 30 days, **stacking from current expiry**) gates the Marketplace `ListItem` flow. Per-character `max_marketplace_listings` column (default 7) replaces the hardcoded constant — future "Listing Slot Extender" SKUs can `+= grant`. Character-select PREMIUM pill, market-panel `PASS Nd` / `NO PASS` status pill, premium-item tooltips. `recall_scroll` purchasable for 500 opals OR 500 gold |
| 🆕 **Chrome system default-on** | 25 per-panel + 3 manager `xxxUseChrome_` toggles flipped to `true`. View-menu items kept as legacy rollback. 13 close-X chrome'd (28 fields). FateStatusBar Menu/Chat/overlay (10 fields). Inter SemiBold across the board, parchment HUD dark dialog scrollbar tab tooltip all reconciled |
...
| **Phase 3C ambient NPC mini-chains shipped** | 30 new quests across 10 chains, applying the state-machine dialogue pattern at scale. Cameo-precedence preserved on overlapping NPC roles via `autoAdvance` ordering. **1599/1599 tests / 10,585 assertions green.** Zero migrations, zero protocol bump — entirely additive content using existing assets |
| **`item_definitions` type-taxonomy hygiene pass** | Closed 4 long-standing data-shape gaps: lowercase `type='material'` (1 row), empty-subtype Materials (17 rows reclassified), `Material/Material` tautology (1 row), `type='Equipment'` orphan ladder (24 rows deleted). DB landed at 816 items across 6 canonical types and 22 Material subtypes. Test seeder patched to `ON CONFLICT DO UPDATE` so future fixture writes self-heal |
| **Server-authoritative PvP zone gate restored** | `combat_handler.cpp` was reading `CharacterStats::isInPvPZone` — a per-character flag that defaulted false and was never written anywhere on the server. PvP was silently disabled game-wide. Wired `inSafeZone` to `sceneCache_.isPvPEnabled(currentScene)`, deleted 3 dead-code paths, restored a broken `MetricsCollector` counter, added 8 new contract tests |
| **Atomic-save swap-conflict fix** | `Inventory::moveItem` is implemented as `std::swap` — both items keep their `instance_id`s, only `slot_index` exchanges. Re-save tripped `uq_character_inventory_slot` partial unique index because Postgres validates per-statement. Fix: NULL the position columns on preserved rows between the orphan-DELETE and UPSERT loop. Two new regression tests cover top-slot + bag-slot swaps |
...
| **PROTOCOL v9 → v20 evolution** | v9: 64-bit ACK window, `SvEntityEnterBatch` coalesce, `CmdAckExtended`, epoch-gated rekey. v10: interact-site packets. v11: site-string-id swap. v12: `CharacterFlags` client mirror. v13: `SvConsumableCooldown`. v14: explicit-stone enchant on the wire. v15: `CmdCraftProtectStone` + `SvCraftProtectStoneResult`. v16: polymorphic `CmdAssignSlot` (skill OR item bindings) + `LoadoutSlotWire` element type + `CmdUseConsumable.source` byte. v17: `CmdShopSellFromBag` for bag-source NPC sells. v18: `SvSkillResultBatch` AOE coalesce. v19: `SvEntityLeaveBatch` AOI deactivation coalesce. v20: `CmdAckExtended` envelope-compatibility bump |
...
- **One pre-existing test failure** — `tests/test_enchant_system.cpp:118-119` expects `mat_enhance_stone_ancient` for Lv 51-60 but the stone-ladder consolidation retired ancient → `getRequiredStone` returns epic. One-line test fix; independent of any active arc.
...
```
engine/                    # 82,195 LOC — Core engine (24 subsystems, 375 files)
 ...
 ui/                       #   65+ widgets (incl. SkillLoadoutStrip w/ drag-bind pipeline, ConfirmDialog chrome, FriendsPanel, BountyPanel), JSON screens, themes, viewport scaling, panel + bespoke widget chrome
...
game/                      # 37,275 LOC — Game logic layer (121 files)
...
server/                    # 35,468 LOC — Headless authoritative server (142 files)
...
tests/                     # 28,402 LOC — 1,687 passing test cases across 229 files (10,973 assertions)
assets/shaders/            #    669 LOC — GLSL + Metal MSL (sprite, post-process, SDF text, lighting)
...
<img src="https://img.shields.io/badge/C%2B%2B-184%2C000%2B_lines-00599C?style=flat-square&logo=cplusplus&logoColor=white" alt="C++ LOC" />
<img src="https://img.shields.io/badge/files-867%2B-2ea44f?style=flat-square" alt="Files" />
