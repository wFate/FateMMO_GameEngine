...
bool PrefabLibrary::saveVariant(const std::string& variantName,
    ...
    // Atomic write to source directory (persists across rebuilds). Mirrors
    // the same hard-failure rule as save() at line 128: a successful runtime
    // write that's NOT mirrored to source will be silently overwritten on
    // the next rebuild, making it indistinguishable from a lost edit. So a
    // source failure has to surface to the caller; reporting success here
    // would re-create the same lying-bug the non-variant path already had
    // to fix.
    if (!sourceDirectory_.empty() && sourceDirectory_ != directory_) {
