...
public:
    ...
    void clearFocus() { if (focusedNode_) { focusedNode_->onFocusLost(); focusedNode_ = nullptr; } }
    void setFocus(UINode* node) {
        if (node != focusedNode_) {
    ...
    // Suppress hot-reload briefly (call after editor save to prevent reload loop)
    void suppressHotReload(float seconds = 1.5f) { hotReloadSuppressTimer_ = seconds; }
...
private:
    ...
    float hotReloadSuppressTimer_ = 0.0f; // skip hot-reload after editor save
