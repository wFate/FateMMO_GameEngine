...
void ShopPanel::render(SpriteBatch& batch, SDFText& sdf) {
    ...
    // ---- Header bar ----
    batch.drawRect({rect.x + rect.w * 0.5f, rect.y + kHeaderH * layoutScale_ * 0.5f},
                   {rect.w, kHeaderH * layoutScale_}, kHeaderBg, d + 0.1f);

    // Shop title
    Vec2 titleSize = sdf.measure(shopName.c_str(), scaledFont(14.0f));
    sdf.drawScreen(batch, shopName.c_str(),
        {rect.x + (rect.w - titleSize.x) * 0.5f, rect.y + (kHeaderH * layoutScale_ - scaledFont(14.0f)) * 0.5f},
        scaledFont(14.0f), kTextColor, d + 0.2f);

    // ---- Close button (X circle, top-right) ----
    float closeR  = 10.0f * layoutScale_;
    float closeCX = rect.x + rect.w - closeR - 5.0f * layoutScale_;
    float closeCY = rect.y + closeR + 5.0f * layoutScale_;
    batch.drawCircle({closeCX, closeCY}, closeR, kCloseBg,  d + 0.2f, 16);
    batch.drawRing  ({closeCX, closeCY}, closeR, 1.0f * layoutScale_, kCloseBdr, d + 0.3f, 16);
    Vec2 xts = sdf.measure("X", scaledFont(10.0f));
    sdf.drawScreen(batch, "X",
        {closeCX - xts.x * 0.5f, closeCY - xts.y * 0.5f},
        scaledFont(10.0f), kCloseXColor, d + 0.4f);

    // ---- Divider below header ----
    batch.drawRect({rect.x + rect.w * 0.5f, rect.y + kHeaderH * layoutScale_},
...
void ShopPanel::renderShopPane(SpriteBatch& batch, SDFText& sdf, float depth) {
    const auto& rect = computedRect_;
    float sPanePad = kPanePad * layoutScale_;
    float sHeaderH = kHeaderH * layoutScale_;
    float sRowH    = kRowH * layoutScale_;
    float paneW = (rect.w - sPanePad * 3.0f) * 0.5f;
    float topY  = rect.y + sHeaderH + sPanePad;

    // Sub-header: "Shop Items"
    sdf.drawScreen(batch, "Shop Items",
        {rect.x + kPanePad * layoutScale_, topY + 3.0f * layoutScale_},
        scaledFont(11.0f), kTextColor, depth + 0.2f);
    ...
    int maxVisible = static_cast<int>(area.h / sRowH);
    int scrollRow = static_cast<int>(scrollOffsetShop_ / sRowH);
    if (scrollRow < 0) scrollRow = 0;
    ...
    for (int i = 0; i < maxVisible; ++i) {
        ...
        float rowY = area.y + static_cast<float>(i) * sRowH;
        float rowCY = rowY + sRowH * 0.5f;

        // Alternate row shading
        if (i % 2 == 1) {
            batch.drawRect({area.x + area.w * 0.5f, rowCY},
                           {area.w, sRowH}, kRowAlt, depth + 0.05f);
        }

        // Item name (left side)
        float nameX = area.x + 4.0f * layoutScale_;
        float nameY = rowY + 4.0f * layoutScale_;
        sdf.drawScreen(batch, entry.itemName.c_str(),
            {nameX, nameY}, scaledFont(10.0f), kTextColor, depth + 0.2f);
        ...
        sdf.drawScreen(batch, priceBuf,
            {nameX, nameY + 13.0f * layoutScale_}, scaledFont(9.0f), kGoldColor, depth + 0.2f);

        // Stock indicator (right of price)
        if (entry.stock >= 0) {
            char stockBuf[24];
            snprintf(stockBuf, sizeof(stockBuf), "x%d", entry.stock);
            float stockX = nameX + 80.0f * layoutScale_;
            sdf.drawScreen(batch, stockBuf,
                {stockX, nameY + 13.0f * layoutScale_}, scaledFont(8.0f), kStockColor, depth + 0.2f);
        }

        // Buy button
        float sBuyBtnW = kBuyBtnW * layoutScale_;
        float sBuyBtnH = kBuyBtnH * layoutScale_;
        float btnX = area.x + area.w - sBuyBtnW - 4.0f * layoutScale_;
        float btnY = rowY + (kRowH * layoutScale_ - sBuyBtnH) * 0.5f;
        float btnCX = btnX + sBuyBtnW * 0.5f;
        float btnCY = btnY + sBuyBtnH * 0.5f;
        ...
        batch.drawRect({btnCX, btnCY}, {sBuyBtnW, sBuyBtnH}, btnColor, depth + 0.15f);

        // Button border
        float bbw = 1.0f;
        Color btnBdr = canBuy ? Color{0.3f, 0.5f, 0.3f, 0.8f} : Color{0.3f, 0.3f, 0.3f, 0.5f};
        float btnInnerH = sBuyBtnH - bbw * 2.0f;
        batch.drawRect({btnCX, btnY + bbw * 0.5f},                  {sBuyBtnW, bbw}, btnBdr, depth + 0.2f);
        batch.drawRect({btnCX, btnY + sBuyBtnH - bbw * 0.5f},       {sBuyBtnW, bbw}, btnBdr, depth + 0.2f);
        batch.drawRect({btnX + bbw * 0.5f, btnCY},                  {bbw, btnInnerH}, btnBdr, depth + 0.2f);
        batch.drawRect({btnX + sBuyBtnW - bbw * 0.5f, btnCY},       {bbw, btnInnerH}, btnBdr, depth + 0.2f);

        // "Buy" label
        Color btnTextColor = canBuy ? Color{0.85f, 0.95f, 0.85f, 1.0f}
                                    : Color{0.5f, 0.5f, 0.5f, 0.7f};
        Vec2 buyTs = sdf.measure("Buy", scaledFont(9.0f));
        sdf.drawScreen(batch, "Buy",
            {btnCX - buyTs.x * 0.5f, btnCY - buyTs.y * 0.5f},
            scaledFont(9.0f), btnTextColor, depth + 0.3f);
    }

    // Vertical divider between panes
    float divX = rect.x + sPanePad + paneW + sPanePad * 0.5f;
    float divY1 = rect.y + sHeaderH + sPanePad;
    float sGoldBarH = kGoldBarH * layoutScale_;
    float divH  = rect.h - sHeaderH - sGoldBarH - sPanePad * 2.0f;
...
void ShopPanel::renderInventoryPane(SpriteBatch& batch, SDFText& sdf, float depth) {
    ...
    float sHeaderH = kHeaderH * layoutScale_;
    float paneW = (rect.w - sPanePad * 3.0f) * 0.5f;
    float rightX = rect.x + sPanePad * 2.0f + paneW;
    float topY   = rect.y + sHeaderH + sPanePad;

    // Sub-header: "Your Inventory"
    sdf.drawScreen(batch, "Your Inventory",
        {rightX, topY + 3.0f * layoutScale_},
        scaledFont(11.0f), kTextColor, depth + 0.2f);
    ...
    float sSlotSize = kSlotSize * layoutScale_;
    float sSlotPad  = kSlotPad * layoutScale_;
    float gridStartX = area.x + sSlotPad;
    float gridStartY = area.y + sSlotPad;
    ...
    for (int row = 0; row < kGridRows; ++row) {
        for (int col = 0; col < kGridCols; ++col) {
            ...
            float cx = gridStartX + sSlotSize * 0.5f + static_cast<float>(col) * (sSlotSize + sSlotPad);
            float cy = gridStartY + sSlotSize * 0.5f + static_cast<float>(row) * (sSlotSize + sSlotPad);

            const InvSlot& slot = playerItems[idx];
            bool hasItem = !slot.itemId.empty();

            // Slot background
            batch.drawRect({cx, cy}, {sSlotSize, sSlotSize}, kSlotBg, depth + 0.1f);

            // Slot border
            float sbw = 1.0f;
            Color slotBdr = {0.25f, 0.25f, 0.35f, 0.6f};
            float sInnerH = sSlotSize - sbw * 2.0f;
            batch.drawRect({cx, cy - sSlotSize * 0.5f + sbw * 0.5f}, {sSlotSize, sbw}, slotBdr, depth + 0.15f);
            batch.drawRect({cx, cy + sSlotSize * 0.5f - sbw * 0.5f}, {sSlotSize, sbw}, slotBdr, depth + 0.15f);
            batch.drawRect({cx - sSlotSize * 0.5f + sbw * 0.5f, cy}, {sbw, sInnerH}, slotBdr, depth + 0.15f);
            batch.drawRect({cx + sSlotSize * 0.5f - sbw * 0.5f, cy}, {sbw, sInnerH}, slotBdr, depth + 0.15f);

            if (hasItem) {
                // Soulbound overlay
                if (slot.soulbound) {
                    batch.drawRect({cx, cy}, {sSlotSize - 2.0f * layoutScale_, sSlotSize - 2.0f * layoutScale_},
                ...
                sdf.drawScreen(batch, label.c_str(),
                    {cx - nameSize.x * 0.5f, cy - 4.0f * layoutScale_},
                ...
                if (slot.quantity > 1) {
                    ...
                    sdf.drawScreen(batch, qbuf,
                        {cx + sSlotSize * 0.5f - 14.0f * layoutScale_, cy + sSlotSize * 0.5f - 10.0f * layoutScale_},
                        scaledFont(8.0f), kQtyColor, depth + 0.25f);
                }
            }
        }
    }
...
void ShopPanel::renderGoldBar(SpriteBatch& batch, SDFText& sdf, float depth) {
    const auto& rect = computedRect_;
    float sGoldBarH = kGoldBarH * layoutScale_;
    float barY  = rect.y + rect.h - sGoldBarH;
    float barCY = barY + sGoldBarH * 0.5f;

    // Divider above
    batch.drawRect({rect.x + rect.w * 0.5f, barY},
                   {rect.w - 4.0f * layoutScale_, 1.0f}, kDividerColor, depth + 0.1f);
    ...
    sdf.drawScreen(batch, goldBuf,
        {rect.x + kPanePad * layoutScale_, barCY - fontSize * 0.5f},
...
void ShopPanel::renderSellConfirm(SpriteBatch& batch, SDFText& sdf, float depth) {
    const auto& rect = computedRect_;
    float sConfirmW = kConfirmW * layoutScale_;
    float sConfirmH = kConfirmH * layoutScale_;
    float popX = rect.x + (rect.w - sConfirmW) * 0.5f;
    float popY = rect.y + (rect.h - sConfirmH) * 0.5f;
    float popCX = popX + sConfirmW * 0.5f;
    float popCY = popY + sConfirmH * 0.5f;
    ...
    // Popup background
    batch.drawRect({popCX, popCY}, {sConfirmW, sConfirmH}, kConfirmBg, depth + 0.6f);

    // Popup border
    float bw = 1.5f;
    float innerH = sConfirmH - bw * 2.0f;
    batch.drawRect({popCX, popY + bw * 0.5f},                     {sConfirmW, bw}, kConfirmBdr, depth + 0.65f);
    batch.drawRect({popCX, popY + sConfirmH - bw * 0.5f},         {sConfirmW, bw}, kConfirmBdr, depth + 0.65f);
    batch.drawRect({popX + bw * 0.5f, popCY},                     {bw, innerH},    kConfirmBdr, depth + 0.65f);
    batch.drawRect({popX + sConfirmW - bw * 0.5f, popCY},         {bw, innerH},    kConfirmBdr, depth + 0.65f);

    // "How many?" label
    Vec2 labelSize = sdf.measure("How many?", scaledFont(12.0f));
    sdf.drawScreen(batch, "How many?",
        {popCX - labelSize.x * 0.5f, popY + 10.0f * layoutScale_},
        scaledFont(12.0f), kTextColor, depth + 0.7f);

    // Quantity display: "- N / max +"
    char qtyBuf[48];
    snprintf(qtyBuf, sizeof(qtyBuf), "%d / %d", sellInputQty_, sellMaxQty_);
    Vec2 qtySize = sdf.measure(qtyBuf, scaledFont(14.0f));
    float qtyY = popY + 35.0f * layoutScale_;
    sdf.drawScreen(batch, qtyBuf,
        {popCX - qtySize.x * 0.5f, qtyY},
        scaledFont(14.0f), kTextColor, depth + 0.7f);

    // "-" button
    float minusBtnX = popCX - qtySize.x * 0.5f - 28.0f * layoutScale_;
    float minusBtnCX = minusBtnX + 10.0f * layoutScale_;
    float minusBtnCY = qtyY + 7.0f * layoutScale_;
    batch.drawRect({minusBtnCX, minusBtnCY}, {20.0f * layoutScale_, 20.0f * layoutScale_}, kConfirmBtnColor, depth + 0.7f);
    Vec2 minusTs = sdf.measure("-", scaledFont(12.0f));
    sdf.drawScreen(batch, "-",
        {minusBtnCX - minusTs.x * 0.5f, minusBtnCY - minusTs.y * 0.5f},
        scaledFont(12.0f), kTextColor, depth + 0.8f);

    // "+" button
    float plusBtnX = popCX + qtySize.x * 0.5f + 8.0f * layoutScale_;
    float plusBtnCX = plusBtnX + 10.0f * layoutScale_;
    float plusBtnCY = qtyY + 7.0f * layoutScale_;
    batch.drawRect({plusBtnCX, plusBtnCY}, {20.0f * layoutScale_, 20.0f * layoutScale_}, kConfirmBtnColor, depth + 0.7f);
    Vec2 plusTs = sdf.measure("+", scaledFont(12.0f));
    sdf.drawScreen(batch, "+",
    ...
    if (sellSlot_ < MAX_SLOTS) {
        ...
        Vec2 spSize = sdf.measure(sellPriceBuf, scaledFont(10.0f));
        sdf.drawScreen(batch, sellPriceBuf,
            {popCX - spSize.x * 0.5f, qtyY + 22.0f * layoutScale_},
            scaledFont(10.0f), kGoldColor, depth + 0.7f);
    }

    // Confirm button
    float btnW = 70.0f * layoutScale_;
    float btnH = 24.0f * layoutScale_;
    float btnY = popY + kConfirmH * layoutScale_ - btnH - 12.0f * layoutScale_;
    float confirmBtnX = popCX - btnW - 8.0f * layoutScale_;
    ...
    Vec2 confTs = sdf.measure("Confirm", scaledFont(10.0f));
    sdf.drawScreen(batch, "Confirm",
        {confirmBtnCX - confTs.x * 0.5f, confirmBtnCY - confTs.y * 0.5f},
        scaledFont(10.0f), kTextColor, depth + 0.8f);

    // Cancel button
    float cancelBtnX = popCX + 8.0f * layoutScale_;
    ...
    Vec2 cancelTs = sdf.measure("Cancel", scaledFont(10.0f));
...
void ShopPanel::renderError(SpriteBatch& batch, SDFText& sdf, float depth) {
    ...
    Vec2 errSize = sdf.measure(errorMessage.c_str(), scaledFont(10.0f));
    float errX = rect.x + (rect.w - errSize.x) * 0.5f;
    float errY = rect.y + rect.h - kGoldBarH * layoutScale_ - 18.0f * layoutScale_;
    sdf.drawScreen(batch, errorMessage.c_str(),
        {errX, errY}, scaledFont(10.0f), errColor, depth + 0.4f);
}
