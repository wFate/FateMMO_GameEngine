...
Entity* EntityFactory::createMob(World& world, const std::string& mobName, int level,
                         int baseHP, int baseDamage, Vec2 spawnPos,
                         bool aggressive, bool isBoss) {
    ...
    if (!sprite->texture) {
        ...
        if (mobName.find("Slime") != std::string::npos) {
        ...
        } else if (mobName.find("squirrel") != std::string::npos) {
            // --- Squirrel: small brown rodent with bushy tail ---
            // Body
            for (int y = 8; y <= 16; y++) for (int x = 6; x <= 16; x++) {
                float dx = x - 11.0f, dy = y - 12.0f;
                if (dx*dx/(6*6) + dy*dy/(5*5) < 1.0f) {
                    int n = mobHash(x, y, 600) % 9 - 4;
                    spMob(x, y, clamp(160+n), clamp(110+n), clamp(60+n));
                }
            }
            // Head
            for (int y = 3; y <= 9; y++) for (int x = 8; x <= 15; x++) {
                float dx = x - 11.5f, dy = y - 6.0f;
                if (dx*dx/(4*4) + dy*dy/(4*4) < 1.0f) {
                    int n = mobHash(x, y, 601) % 7 - 3;
                    spMob(x, y, clamp(170+n), clamp(120+n), clamp(70+n));
                }
            }
            // Ears
            spMob(9, 2, 150, 105, 55); spMob(14, 2, 150, 105, 55);
            // Eyes
            spMob(10, 5, 20, 20, 20); spMob(13, 5, 20, 20, 20);
            // Nose
            spMob(12, 7, 40, 30, 25);
            // Bushy tail (curving up-right)
            for (int y = 4; y <= 14; y++) {
                int tx = 17 + (14 - y) / 3;
                int n = mobHash(tx, y, 602) % 7 - 3;
                spMob(tx, y, clamp(165+n), clamp(115+n), clamp(55+n));
                spMob(tx+1, y, clamp(155+n), clamp(105+n), clamp(50+n));
            }
            // Legs
            for (int y = 16; y <= 21; y++) {
                int n = mobHash(8, y, 603) % 5 - 2;
                spMob(8, y, clamp(150+n), clamp(100+n), clamp(55+n));
                spMob(9, y, clamp(150+n), clamp(100+n), clamp(55+n));
                spMob(13, y, clamp(150+n), clamp(100+n), clamp(55+n));
                spMob(14, y, clamp(150+n), clamp(100+n), clamp(55+n));
            }
            // Outline
            for (int y = 0; y < SZ; y++) for (int x = 0; x < SZ; x++) {
                if (pixels[(y*SZ+x)*4+3] == 0) continue;
                for (int dy=-1;dy<=1;dy++) for (int dx=-1;dx<=1;dx++) {
                    int nx=x+dx, ny=y+dy;
                    if (nx>=0 && nx<SZ && ny>=0 && ny<SZ && pixels[(ny*SZ+nx)*4+3]==0)
                        { outlineMob(x,y); goto squirrel_next; }
                }
                squirrel_next:;
            }

        } else if (mobName.find("giant_rat") != std::string::npos) {
            // --- Giant Rat: dark grey rodent, long tail ---
            // Body (horizontal ellipse)
            for (int y = 8; y <= 16; y++) for (int x = 4; x <= 18; x++) {
                float dx = x - 11.0f, dy = y - 12.0f;
                if (dx*dx/(8*8) + dy*dy/(5*5) < 1.0f) {
                    int n = mobHash(x, y, 610) % 9 - 4;
                    spMob(x, y, clamp(90+n), clamp(80+n), clamp(75+n));
                }
            }
            // Head
            for (int y = 6; y <= 12; y++) for (int x = 15; x <= 22; x++) {
                float dx = x - 18.5f, dy = y - 9.0f;
                if (dx*dx/(4*4) + dy*dy/(4*4) < 1.0f) {
                    int n = mobHash(x, y, 611) % 7 - 3;
                    spMob(x, y, clamp(100+n), clamp(90+n), clamp(85+n));
                }
            }
            // Red eyes
            spMob(20, 8, 200, 40, 30); spMob(21, 8, 220, 50, 35);
            // Nose/snout
            spMob(22, 9, 180, 120, 120); spMob(23, 9, 160, 100, 100);
            // Ears
            spMob(17, 5, 110, 95, 90); spMob(19, 5, 110, 95, 90);
            // Tail (long, curving left)
            for (int i = 0; i < 8; i++) {
                int tx = 3 - i/2, ty = 13 - i;
                spMob(tx+1, ty, 120, 100, 95); spMob(tx, ty, 110, 90, 85);
            }
            // Legs
            for (int y = 16; y <= 21; y++) {
                int n = mobHash(6, y, 612) % 5 - 2;
                spMob(6, y, clamp(85+n), clamp(75+n), clamp(70+n));
                spMob(7, y, clamp(85+n), clamp(75+n), clamp(70+n));
                spMob(14, y, clamp(85+n), clamp(75+n), clamp(70+n));
                spMob(15, y, clamp(85+n), clamp(75+n), clamp(70+n));
            }
            // Outline
            for (int y = 0; y < SZ; y++) for (int x = 0; x < SZ; x++) {
                if (pixels[(y*SZ+x)*4+3] == 0) continue;
                for (int dy=-1;dy<=1;dy++) for (int dx=-1;dx<=1;dx++) {
                    int nx=x+dx, ny=y+dy;
                    if (nx>=0 && nx<SZ && ny>=0 && ny<SZ && pixels[(ny*SZ+nx)*4+3]==0)
                        { outlineMob(x,y); goto rat_next; }
                }
                rat_next:;
            }

        } else if (mobName.find("horned_hare") != std::string::npos) {
            // --- Horned Hare: white rabbit with small antlers ---
            // Body
            for (int y = 9; y <= 17; y++) for (int x = 7; x <= 16; x++) {
                float dx = x - 11.5f, dy = y - 13.0f;
                if (dx*dx/(5*5) + dy*dy/(5*5) < 1.0f) {
                    int n = mobHash(x, y, 620) % 7 - 3;
                    spMob(x, y, clamp(220+n), clamp(215+n), clamp(210+n));
                }
            }
            // Head
            for (int y = 3; y <= 10; y++) for (int x = 8; x <= 15; x++) {
                float dx = x - 11.5f, dy = y - 6.5f;
                if (dx*dx/(4*4) + dy*dy/(4*4) < 1.0f) {
                    int n = mobHash(x, y, 621) % 5 - 2;
                    spMob(x, y, clamp(230+n), clamp(225+n), clamp(220+n));
                }
            }
            // Long ears
            for (int y = 0; y <= 4; y++) {
                spMob(9, y, 225, 220, 215); spMob(10, y, 230, 225, 220);
                spMob(13, y, 225, 220, 215); spMob(14, y, 230, 225, 220);
            }
            // Inner ear pink
            spMob(10, 1, 220, 170, 170); spMob(10, 2, 215, 165, 165);
            spMob(13, 1, 220, 170, 170); spMob(13, 2, 215, 165, 165);
            // Horns (small brown antlers between ears)
            spMob(11, 0, 140, 100, 60); spMob(11, 1, 130, 90, 50);
            spMob(12, 0, 140, 100, 60); spMob(12, 1, 130, 90, 50);
            spMob(10, 0, 120, 85, 45); spMob(13, 0, 120, 85, 45);
            // Eyes
            spMob(10, 6, 180, 40, 40); spMob(13, 6, 180, 40, 40);
            // Nose
            spMob(11, 8, 200, 150, 150); spMob(12, 8, 200, 150, 150);
            // Legs
            for (int y = 17; y <= 22; y++) {
                int n = mobHash(9, y, 622) % 5 - 2;
                spMob(9, y, clamp(215+n), clamp(210+n), clamp(205+n));
                spMob(10, y, clamp(215+n), clamp(210+n), clamp(205+n));
                spMob(13, y, clamp(215+n), clamp(210+n), clamp(205+n));
                spMob(14, y, clamp(215+n), clamp(210+n), clamp(205+n));
            }
            // Fluffy tail
            spMob(7, 14, 235, 230, 225); spMob(6, 13, 230, 225, 220);
            spMob(6, 14, 235, 230, 225); spMob(7, 13, 240, 235, 230);
            // Outline
            for (int y = 0; y < SZ; y++) for (int x = 0; x < SZ; x++) {
                if (pixels[(y*SZ+x)*4+3] == 0) continue;
                for (int dy=-1;dy<=1;dy++) for (int dx=-1;dx<=1;dx++) {
                    int nx=x+dx, ny=y+dy;
                    if (nx>=0 && nx<SZ && ny>=0 && ny<SZ && pixels[(ny*SZ+nx)*4+3]==0)
                        { outlineMob(x,y); goto hare_next; }
                }
                hare_next:;
            }

        } else if (mobName.find("timber_wolf") != std::string::npos || mobName.find("timber_alpha") != std::string::npos) {
            // --- Timber Wolf/Alpha: darker, larger wolf variant ---
            bool isAlpha = mobName.find("alpha") != std::string::npos;
            int darkShift = isAlpha ? -20 : 0;
            // Body
            for (int y = 6; y <= 16; y++) for (int x = 3; x <= 20; x++) {
                float dx = x - 11.5f, dy = y - 11.0f;
                if (dx*dx/(9*9) + dy*dy/(5.5f*5.5f) < 1.0f) {
                    int n = mobHash(x, y, 630) % 9 - 4;
                    int shade = (y < 10) ? 8 : -5;
                    spMob(x, y, clamp(100+n+shade+darkShift), clamp(95+n+shade+darkShift), clamp(90+n+shade+darkShift));
                }
            }
            // Head
            for (int y = 5; y <= 12; y++) for (int x = 17; x <= 23; x++) {
                float dx = x - 20.0f, dy = y - 8.5f;
                if (dx*dx/(4*4) + dy*dy/(4*4) < 1.0f) {
                    int n = mobHash(x, y, 631) % 7 - 3;
                    spMob(x, y, clamp(110+n+darkShift), clamp(105+n+darkShift), clamp(100+n+darkShift));
                }
            }
            // Ears
            spMob(19, 3, clamp(80+darkShift), clamp(75+darkShift), clamp(70+darkShift));
            spMob(19, 4, clamp(90+darkShift), clamp(85+darkShift), clamp(80+darkShift));
            spMob(21, 3, clamp(80+darkShift), clamp(75+darkShift), clamp(70+darkShift));
            spMob(21, 4, clamp(90+darkShift), clamp(85+darkShift), clamp(80+darkShift));
            // Eyes (amber for wolf, red for alpha)
            if (isAlpha) {
                spMob(21, 8, 220, 50, 30); spMob(22, 8, 240, 60, 35);
            } else {
                spMob(21, 8, 200, 180, 40); spMob(22, 8, 210, 190, 50);
            }
            // Nose + mouth
            spMob(23, 9, 40, 35, 35);
            spMob(22, 10, 80, 50, 50); spMob(23, 10, 80, 50, 50);
            // Legs
            for (int y = 16; y <= 22; y++) {
                int n = mobHash(5, y, 632) % 5 - 2;
                unsigned char c = clamp(90+n+darkShift);
                spMob(5, y, c, clamp(c-5), clamp(c-10));
                spMob(6, y, c, clamp(c-5), clamp(c-10));
                spMob(9, y, clamp(c+5), c, clamp(c-5));
                spMob(10, y, clamp(c+5), c, clamp(c-5));
                spMob(15, y, clamp(c+5), c, clamp(c-5));
                spMob(16, y, clamp(c+5), c, clamp(c-5));
                spMob(19, y, c, clamp(c-5), clamp(c-10));
                spMob(20, y, c, clamp(c-5), clamp(c-10));
            }
            // Tail
            spMob(2, 8, clamp(95+darkShift), clamp(90+darkShift), clamp(85+darkShift));
            spMob(1, 7, clamp(95+darkShift), clamp(90+darkShift), clamp(85+darkShift));
            spMob(3, 9, clamp(100+darkShift), clamp(95+darkShift), clamp(90+darkShift));
            // Outline
            for (int y = 0; y < SZ; y++) for (int x = 0; x < SZ; x++) {
                if (pixels[(y*SZ+x)*4+3] == 0) continue;
                for (int dy=-1;dy<=1;dy++) for (int dx=-1;dx<=1;dx++) {
                    int nx=x+dx, ny=y+dy;
                    if (nx>=0 && nx<SZ && ny>=0 && ny<SZ && pixels[(ny*SZ+nx)*4+3]==0)
                        { outlineMob(x,y); goto twolf_next; }
                }
                twolf_next:;
            }

        } else if (mobName.find("grizzly_bear") != std::string::npos) {
            // --- Grizzly Bear: large dark brown bear ---
            // Body (wide)
            for (int y = 6; y <= 18; y++) for (int x = 4; x <= 19; x++) {
                float dx = x - 11.5f, dy = y - 12.0f;
                if (dx*dx/(8*8) + dy*dy/(7*7) < 1.0f) {
                    int n = mobHash(x, y, 640) % 9 - 4;
                    spMob(x, y, clamp(100+n), clamp(70+n), clamp(40+n));
                }
            }
            // Head
            for (int y = 2; y <= 9; y++) for (int x = 7; x <= 16; x++) {
                float dx = x - 11.5f, dy = y - 5.5f;
                if (dx*dx/(5*5) + dy*dy/(4*4) < 1.0f) {
                    int n = mobHash(x, y, 641) % 7 - 3;
                    spMob(x, y, clamp(110+n), clamp(78+n), clamp(45+n));
                }
            }
            // Ears
            spMob(8, 1, 90, 60, 35); spMob(9, 1, 95, 65, 38);
            spMob(14, 1, 90, 60, 35); spMob(15, 1, 95, 65, 38);
            // Eyes
            spMob(10, 5, 20, 15, 10); spMob(13, 5, 20, 15, 10);
            // Nose
            spMob(11, 7, 40, 30, 25); spMob(12, 7, 40, 30, 25);
            // Snout lighter patch
            for (int x = 10; x <= 13; x++) { spMob(x, 8, 140, 105, 65); }
            // Legs (thick)
            for (int y = 18; y <= 23; y++) {
                int n = mobHash(6, y, 642) % 5 - 2;
                for (int x = 5; x <= 8; x++) spMob(x, y, clamp(90+n), clamp(62+n), clamp(35+n));
                for (int x = 15; x <= 18; x++) spMob(x, y, clamp(90+n), clamp(62+n), clamp(35+n));
            }
            // Claws
            spMob(5, 23, 200, 195, 185); spMob(6, 23, 200, 195, 185);
            spMob(17, 23, 200, 195, 185); spMob(18, 23, 200, 195, 185);
            // Outline
            for (int y = 0; y < SZ; y++) for (int x = 0; x < SZ; x++) {
                if (pixels[(y*SZ+x)*4+3] == 0) continue;
                for (int dy=-1;dy<=1;dy++) for (int dx=-1;dx<=1;dx++) {
...
Entity* EntityFactory::createNPC(World& world, const NPCTemplate& tmpl) {
...
void EntityFactory::ensureMobSprite(World& world, const std::string& mobId, bool isBoss) {
...
Entity* EntityFactory::createGhostMob(World& world, const std::string& name, Vec2 position,
                               const std::string& mobDefId, int level,
...
Entity* EntityFactory::createDroppedItem(World& world, Vec2 position, bool isGold) {
...
Entity* EntityFactory::createGhostDroppedItem(World& world, const std::string& name, Vec2 position,
                                       bool isGold, const std::string& rarity) {
    Entity* entity = world.createEntity(name);
    entity->setTag("dropped_item");
