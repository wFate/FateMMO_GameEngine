...
void GameApp::onInit() {
    ...
    netClient_.onCombatEvent = [this](const SvCombatEventMsg& msg) {
        ...
        // Show floating damage text for non-local attacks (mob→player, remote→mob).
        // Local player→mob text is already shown by CombatActionSystem prediction.
        if (!isLocalAttack) {
            if (msg.damage == 0) {
            ...
            if (netClient_.isConnected()) {
                ...
                if (pendingZoneTransition_) {
                    ...
                    if (sc) {
                        ...
                        // Restore player state from last SvPlayerState (level, HP, etc.)
                        auto* cs = player->getComponent<CharacterStatsComponent>();
                        if (cs) {
                            const auto& ps = pendingPlayerState_;
                            cs->stats.level = ps.level;
