...
inline void registerAllComponents() {
        ...
        [](const void* data, nlohmann::json& j) {
            ...
            for (const auto& d : c->destinations) {
                dests.push_back({
                    ...
                    {"requiredItemQty", d.requiredItemQty}
                });
            }
        },
        [](const nlohmann::json& j, void* data) {
            ...
            if (j.contains("destinations")) {
                ...
                for (const auto& d : j["destinations"]) {
                    ...
                    if (d.contains("requiredItem"))    td.requiredItem    = d["requiredItem"].get<std::string>();
                    if (d.contains("requiredItemQty")) td.requiredItemQty = d["requiredItemQty"].get<uint16_t>();
    ...
    // DungeonNPCComponent: simple string field, auto-serialized via FATE_REFLECT
    reg.registerComponent<DungeonNPCComponent>();

    // ArenaNPCComponent: marker only, auto-serialized
    reg.registerComponent<ArenaNPCComponent>();
