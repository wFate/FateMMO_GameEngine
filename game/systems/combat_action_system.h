...
public:
    ...
    void renderFloatingTexts(SpriteBatch& batch, Camera& camera, SDFText& sdf) {
        ...
        constexpr float SEL_DEPTH = 95.0f;  // selection marker
        ...
        if (currentTargetId_ != INVALID_ENTITY && world_) {
            ...
            if (target) {
                ...
                if (targetT && targetS && targetS->enabled) {
                    ...
                    Color markerColor(1.0f, 0.3f, 0.3f, pulse);
                    Vec2 pos = targetT->position;
                    Vec2 sz = targetS->size * 1.3f;
                    float t = 1.5f;
                    batch.drawRect({pos.x, pos.y + sz.y * 0.5f}, {sz.x, t}, markerColor, SEL_DEPTH);
                    batch.drawRect({pos.x, pos.y - sz.y * 0.5f}, {sz.x, t}, markerColor, SEL_DEPTH);
                    batch.drawRect({pos.x - sz.x * 0.5f, pos.y}, {t, sz.y}, markerColor, SEL_DEPTH);
                    batch.drawRect({pos.x + sz.x * 0.5f, pos.y}, {t, sz.y}, markerColor, SEL_DEPTH);
                }
            }
        }
                ...
                [&](Entity* entity, Transform* t, NameplateComponent* np) {
                    ...
                    if (qm && qm->currentState != MarkerState::None) {
                        ...
                        Color markerCol = (qm->currentState == MarkerState::TurnIn)
                            ? Color(1.0f, 0.86f, 0.0f, 1.0f) : Color::white();
...
private:
    ...
    enum class AttackPose { Melee, BowDraw, Channel };
    ...
    void processClickTargeting() {
        ...
        if (Editor::instance().isOpen()) {
            ...
            if (!Editor::instance().inPlayMode()) return;
        ...
        if (playerAnim) {
            ...
            if (isArcher) {
                ...
                pendingReleaseCallback_ = [this, pending]() {
                    if (onPlaySFX) {
                        if (pending.isCrit) onPlaySFX("hit_crit");
                        else onPlaySFX("hit_melee");
                    }
                };
