...
public:
    ...
    const auto& allTables() const { return tables_; }
    bool hasTable(const std::string& id) const { return tables_.count(id) > 0; }
    void reload(pqxx::connection& conn) { initialize(conn, *itemDefs_); }
