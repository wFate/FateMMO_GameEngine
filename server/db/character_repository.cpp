...
CharacterRecord CharacterRepository::rowToRecord(const pqxx::row& row) {
    ...
    try { rec.recall_scene = row["recall_scene"].is_null() ? "Town" : row["recall_scene"].as<std::string>(); } catch (...) {}

    // Stat allocation (gracefully handle missing columns for DB migration)
    try { rec.free_stat_points = row["free_stat_points"].is_null() ? 0 : row["free_stat_points"].as<int>(); } catch (...) {}
    try { rec.allocated_str    = row["allocated_str"].is_null()    ? 0 : row["allocated_str"].as<int>();    } catch (...) {}
    try { rec.allocated_int    = row["allocated_int"].is_null()    ? 0 : row["allocated_int"].as<int>();    } catch (...) {}
    try { rec.allocated_dex    = row["allocated_dex"].is_null()    ? 0 : row["allocated_dex"].as<int>();    } catch (...) {}
    try { rec.allocated_con    = row["allocated_con"].is_null()    ? 0 : row["allocated_con"].as<int>();    } catch (...) {}
    try { rec.allocated_wis    = row["allocated_wis"].is_null()    ? 0 : row["allocated_wis"].as<int>();    } catch (...) {}
...
std::optional<CharacterRecord> CharacterRepository::loadCharacter(const std::string& characterId) {
    try {
        ...
        auto result = txn.exec_params(
            ...
            "pk_status, faction, recall_scene, "
            "free_stat_points, allocated_str, allocated_int, allocated_dex, allocated_con, allocated_wis "
...
std::vector<CharacterRecord> CharacterRepository::loadCharactersByAccount(int accountId) {
    ...
    try {
        ...
        auto result = txn.exec_params(
            ...
            "pk_status, faction, recall_scene, "
            "free_stat_points, allocated_str, allocated_int, allocated_dex, allocated_con, allocated_wis "
            "FROM characters WHERE account_id = $1 ORDER BY character_id LIMIT 3",
            accountId);
        txn.commit();
        ...
        LOG_ERROR("CharRepo", "loadCharactersByAccount failed: %s", e.what());
    }
    return records;
}

bool CharacterRepository::deleteCharacter(const std::string& characterId, int accountId) {
    try {
        auto guard = acquireConn();
        pqxx::work txn(guard.connection());
        // Verify ownership
        ...
        txn.exec_params("DELETE FROM character_skills WHERE character_id = $1", characterId);
        txn.exec_params("DELETE FROM characters WHERE character_id = $1", characterId);
        txn.commit();
        return true;
    } catch (const std::exception& e) {
        LOG_ERROR("CharRepo", "deleteCharacter failed: %s", e.what());
        return false;
    }
}
