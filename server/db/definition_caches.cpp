...
bool SkillDefCache::initialize(pqxx::connection& conn) {
    try {
        // Auto-migrate: add vfx_id column if it doesn't exist yet
        bool hasVfxId = false;
        try {
            pqxx::work migrateTxn(conn);
            migrateTxn.exec("ALTER TABLE skill_definitions ADD COLUMN IF NOT EXISTS vfx_id TEXT DEFAULT NULL");
            migrateTxn.commit();
            hasVfxId = true;
        } catch (...) {
            // Column migration failed (e.g. permissions) — proceed without vfx_id
        }
        ...
        if (hasVfxId) sql += ", vfx_id";
        sql += " FROM skill_definitions ORDER BY class_req, level_required";
        auto skillResult = txn.exec(sql);
        ...
        for (const auto& row : skillResult) {
            ...
            if (hasVfxId) {
                d.vfxId = row["vfx_id"].is_null() ? "" : row["vfx_id"].as<std::string>();
            }
...
bool SceneCache::initialize(pqxx::connection& conn) {
    try {
        // Auto-migrate: add spawn point columns if they don't exist
        {
            pqxx::work migrateTxn(conn);
        ...
        auto result = txn.exec(
            ...
            "COALESCE(is_aurora, false) AS is_aurora "
            "FROM scenes");
        txn.commit();
        ...
        for (const auto& row : result) {
            ...
            s.isAurora = row["is_aurora"].as<bool>(false);
