...
public:
    ClientRateLimiter() {
        ...
        configure(PacketType::CmdShopBuy,         3.0f,  1.0f,   30);
        configure(PacketType::CmdShopSell,        3.0f,  1.0f,   30);
        configure(PacketType::CmdTeleport,        2.0f,  0.5f,   20);
        configure(PacketType::CmdBankDepositItem,  3.0f,  1.0f,   30);
        configure(PacketType::CmdBankWithdrawItem, 3.0f,  1.0f,   30);
        configure(PacketType::CmdBankDepositGold,  3.0f,  1.0f,   30);
        configure(PacketType::CmdBankWithdrawGold, 3.0f,  1.0f,   30);
        ...
        configure(PacketType::CmdEquip,            3.0f,  1.0f,   30);
        configure(PacketType::CmdEnchant,          2.0f,  0.5f,   30);
        configure(PacketType::CmdRepair,           2.0f,  0.5f,   30);
        configure(PacketType::CmdExtractCore,      2.0f,  0.5f,   30);
        configure(PacketType::CmdCraft,            2.0f,  0.5f,   30);
        configure(PacketType::CmdBattlefield,      2.0f,  0.5f,   20);
        configure(PacketType::CmdArena,            2.0f,  0.5f,   20);
        configure(PacketType::CmdPet,              2.0f,  0.5f,   20);
        configure(PacketType::CmdStartDungeon,     2.0f,  0.5f,   20);
        configure(PacketType::CmdDungeonResponse,  2.0f,  0.5f,   20);
        configure(PacketType::CmdMoveItem,         5.0f,  2.0f,   50);
        configure(PacketType::CmdDestroyItem,      3.0f,  1.0f,   30);
    }
    ...
    // Add weighted violation (e.g., malformed packets count heavier).
    // Reuses the same 60s decay window as rate-limit violations.
    void addViolation(double now, uint32_t weight = 1) {
        // Decay if 60s since last violation
        if (violations_ > 0 && (now - lastViolationTime_) >= 60.0) {
            violations_ = 0;
        }
        violations_ += weight;
