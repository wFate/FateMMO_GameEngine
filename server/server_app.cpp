...
bool ServerApp::init(uint16_t port) {
    ...
    // Wire content validator to caches and run startup validation
    contentValidator_.setCaches(&itemDefCache_, &mobDefCache_, &lootTableCache_,
                                &spawnZoneCache_, &skillDefCache_);
    {
        auto issues = contentValidator_.runAll();
        int errors = 0, warnings = 0;
        for (const auto& issue : issues) {
            if (issue.severity == 0) { LOG_ERROR("Validator", "%s", issue.message.c_str()); errors++; }
            else if (issue.severity == 1) { LOG_WARN("Validator", "%s", issue.message.c_str()); warnings++; }
        }
        LOG_INFO("Validator", "Content validation: %d errors, %d warnings, %zu total",
                 errors, warnings, issues.size());
    }
...
// ---------------------------------------------------------------------------
// Load NPC entities from scene JSON into the server world
// Only loads entities that have NPCComponent (shops, bankers, teleporters, etc.)
// ---------------------------------------------------------------------------
void ServerApp::loadNPCsFromScenes() {
    const std::string scenesDir = "assets/scenes/";
    int npcCount = 0;
    ...
    for (const auto& filePath : sceneFiles) {
        std::string filename = filePath.substr(filePath.find_last_of("/\\") + 1);
        std::string sceneId = filename.substr(0, filename.size() - 5);
        ...
        for (const auto& entityDef : *entitiesIt) {
            auto compsIt = entityDef.find("components");
            if (compsIt == entityDef.end()) continue;

            // Only load entities that have NPCComponent
            if (compsIt->find("NPCComponent") == compsIt->end()) continue;

            auto transformIt = compsIt->find("Transform");
            if (transformIt == compsIt->end()) continue;

            // Parse NPCComponent
            auto& npcJson = (*compsIt)["NPCComponent"];
            uint32_t npcId = npcJson.value("npcId", 0u);
            if (npcId == 0) continue;
            ...
            auto shopIt = compsIt->find("ShopComponent");
            auto bankIt = compsIt->find("BankerComponent");
            auto teleIt = compsIt->find("TeleporterComponent");

            // Add ALL components first (addComponent can invalidate prior pointers
            // due to archetype migration), then populate data afterward.
            e->addComponent<Transform>();
            e->addComponent<NPCComponent>();
            if (shopIt != compsIt->end()) e->addComponent<ShopComponent>();
            if (bankIt != compsIt->end()) e->addComponent<BankerComponent>();
            if (teleIt != compsIt->end()) e->addComponent<TeleporterComponent>();

            // Now populate — re-fetch pointers after all archetype changes are done
            auto* t = e->getComponent<Transform>();
            if (t) {
                auto posIt = transformIt->find("position");
                if (posIt != transformIt->end() && posIt->is_array() && posIt->size() >= 2) {
                    t->position.x = (*posIt)[0].get<float>();
                    t->position.y = (*posIt)[1].get<float>();
                }
            }

            auto* nc = e->getComponent<NPCComponent>();
            if (nc) {
                nc->npcId             = npcId;
                nc->displayName       = npcJson.value("displayName", "");
                nc->dialogueGreeting  = npcJson.value("dialogueGreeting", "");
                nc->sceneId           = npcJson.value("sceneId", sceneId);
                nc->interactionRadius = npcJson.value("interactionRadius", 2.0f);
            }

            if (shopIt != compsIt->end()) {
                auto* sc = e->getComponent<ShopComponent>();
                if (sc) {
                    sc->shopName = shopIt->value("shopName", "Shop");
                    if (shopIt->contains("inventory") && (*shopIt)["inventory"].is_array()) {
                        for (const auto& item : (*shopIt)["inventory"]) {
                            ShopItem si;
                            si.itemId    = item.value("itemId", "");
                            si.itemName  = item.value("itemName", "");
                            si.buyPrice  = item.value("buyPrice", static_cast<int64_t>(0));
                            si.sellPrice = item.value("sellPrice", static_cast<int64_t>(0));
                            si.stock     = item.value("stock", static_cast<uint16_t>(0));
                            if (!si.itemId.empty()) sc->inventory.push_back(std::move(si));
                        }
                    }
                }
            }

            if (bankIt != compsIt->end()) {
                auto* bc = e->getComponent<BankerComponent>();
                if (bc) {
                    bc->storageSlots = bankIt->value("storageSlots", static_cast<uint16_t>(30));
                }
            }

            if (teleIt != compsIt->end()) {
                auto* tc = e->getComponent<TeleporterComponent>();
                if (tc && teleIt->contains("destinations") && (*teleIt)["destinations"].is_array()) {
                    for (const auto& dest : (*teleIt)["destinations"]) {
                        TeleportDestination td;
                        td.destinationName = dest.value("destinationName", "");
                        td.sceneId         = dest.value("sceneId", "");
                        td.cost            = dest.value("cost", static_cast<int64_t>(0));
                        td.requiredLevel   = dest.value("requiredLevel", static_cast<uint16_t>(0));
                        td.requiredItem    = dest.value("requiredItem", "");
                        td.requiredItemQty = dest.value("requiredItemQty", static_cast<uint16_t>(0));
                        if (dest.contains("targetPosition") && dest["targetPosition"].is_array() && dest["targetPosition"].size() >= 2) {
                            td.targetPosition.x = dest["targetPosition"][0].get<float>();
                            td.targetPosition.y = dest["targetPosition"][1].get<float>();
                        }
                        tc->destinations.push_back(std::move(td));
                    }
                }
            }

            std::string npcName = npcJson.value("displayName", "");
            ++npcCount;
            LOG_DEBUG("Server", "Loaded NPC '%s' (id=%u) in scene '%s'",
                      npcName.c_str(), npcId, sceneId.c_str());
        }
    }

    LOG_INFO("Server", "Loaded %d NPC entities from scene files", npcCount);
}
...
void ServerApp::tick(float dt) {
    ...
    auto tp4 = Clock::now(); // after despawn
...
void ServerApp::onClientConnected(uint16_t clientId) {
    ...
    if (charStatsComp) {
        ...
        // Sync FactionComponent to match DB-loaded faction (createPlayer passes Faction::None)
        auto* factionComp = player->getComponent<FactionComponent>();
        if (factionComp) {
            factionComp->faction = s.faction;
        }
    ...
    if (inv) {
        ...
        int equippedCount = 0;
        for (const auto& slot : invSlots) {
            if (slot.is_equipped) equippedCount++;
        }
        LOG_INFO("Server", "Loaded %zu inventory items for %s (%d equipped, %d gold)",
                 invSlots.size(), rec.character_name.c_str(), equippedCount, rec.gold);

        // Log each equipped item by slot
        auto equipMapLog = inv->inventory.getEquipmentMap();
        for (const auto& [eqSlot, eqItem] : equipMapLog) {
            if (!eqItem.isValid()) continue;
            const char* slotStr = "Unknown";
            switch (eqSlot) {
                case EquipmentSlot::Weapon:    slotStr = "Weapon"; break;
                case EquipmentSlot::SubWeapon: slotStr = "SubWeapon"; break;
                case EquipmentSlot::Hat:       slotStr = "Hat"; break;
                case EquipmentSlot::Armor:     slotStr = "Armor"; break;
                case EquipmentSlot::Gloves:    slotStr = "Gloves"; break;
                case EquipmentSlot::Shoes:     slotStr = "Shoes"; break;
                case EquipmentSlot::Belt:      slotStr = "Belt"; break;
                case EquipmentSlot::Cloak:     slotStr = "Cloak"; break;
                case EquipmentSlot::Ring:      slotStr = "Ring"; break;
                case EquipmentSlot::Necklace:  slotStr = "Necklace"; break;
                default: break;
            }
            if (eqItem.enchantLevel > 0) {
                LOG_INFO("Server", "  Equipped %s: '%s' +%d", slotStr,
                         eqItem.displayName.c_str(), eqItem.enchantLevel);
            } else {
                LOG_INFO("Server", "  Equipped %s: '%s'", slotStr,
                         eqItem.displayName.c_str());
            }
        }
    ...
    if (guildComp) {
        ...
        if (guildId > 0) {
            ...
            if (guildInfo) {
                ...
                LOG_INFO("Server", "Loaded guild '%s' (id=%d, rank=%d, level=%d) for %s",
                         guildInfo->guildName.c_str(), guildInfo->guildId, rank,
                         guildInfo->guildLevel, rec.character_name.c_str());
            }
        }
    }
    ...
    if (questComp) {
        ...
        LOG_INFO("Server", "Loaded %zu active quests, %zu completed for %s",
                 activeQuests.size(), completedIds.size(), rec.character_name.c_str());
    }
    ...
    if (bankComp) {
        ...
        if (bankGold > 0) {
            LOG_INFO("Server", "Loaded bank for %s (%lld gold stored)",
                     rec.character_name.c_str(), static_cast<long long>(bankGold));
        }
    }
    ...
    if (petComp) {
        ...
        if (equippedPet) {
            ...
            LOG_INFO("Server", "Loaded pet '%s' (level %d) for %s",
                     equippedPet->petName.c_str(), equippedPet->level,
                     rec.character_name.c_str());
        }
    }
    ...
    // Load completed collections
    {
        ...
        if (!completedColIds.empty()) {
            LOG_INFO("Server", "Loaded %zu completed collections for %s",
                     completedColIds.size(), rec.character_name.c_str());
        }
    ...
    {
        std::string guildStr = "none";
        if (guildComp && guildComp->guild.guildId > 0) {
            guildStr = "'" + guildComp->guild.guildName + "'";
        }
        LOG_INFO("Server", "Client %d connected: account=%d char='%s' level=%d class=%s guild=%s scene=%s",
                 clientId, session.account_id, rec.character_name.c_str(), rec.level,
                 rec.class_name.c_str(), guildStr.c_str(), rec.current_scene.c_str());
    }
}
...
void ServerApp::onClientDisconnected(uint16_t clientId) {
    ...
    // Clear invite prompt state on disconnect
    if (client) {
    ...
    if (client && client->playerEntityId != 0) {
        ...
        // Clean up party membership on disconnect — notify remaining members
        {
            World& w = getWorldForClient(clientId);
            ...
            if (pe) {
                ...
                if (partyComp && partyComp->party.isInParty()) {
                    std::string leaverCharId = client->character_id;

                    std::vector<std::string> memberCharIds;
                    for (const auto& m : partyComp->party.members) {
                        if (m.characterId != leaverCharId)
                            memberCharIds.push_back(m.characterId);
                    }

                    partyComp->party.leaveParty();

                    // Build update message
                    SvPartyUpdateMsg update;
                    update.event = PartyEvent::Left;
                    update.actorCharId = leaverCharId;

                    // Find new party state from any remaining member
                    for (const auto& mCharId : memberCharIds) {
                        server_.connections().forEach([&](ClientConnection& c) {
                            if (c.character_id == mCharId && c.playerEntityId != 0) {
                                PersistentId mp(c.playerEntityId);
                                EntityHandle mh = getReplicationForClient(c.clientId).getEntityHandle(mp);
                                Entity* me = getWorldForClient(c.clientId).getEntity(mh);
                                if (me) {
                                    auto* mParty = me->getComponent<PartyComponent>();
                                    if (mParty && mParty->party.isInParty()) {
                                        // Also remove from their local state
                                        mParty->party.removeMember(leaverCharId);
                                        update.lootMode = static_cast<uint8_t>(mParty->party.lootMode);
                                        for (const auto& pm : mParty->party.members) {
                                            PartyMemberNetInfo info;
                                            info.charId = pm.characterId;
                                            info.name = pm.characterName;
                                            info.level = static_cast<uint8_t>(pm.level);
                                            info.hp = pm.currentHP; info.maxHp = pm.maxHP;
                                            info.mp = pm.currentMP; info.maxMp = pm.maxMP;
                                            update.members.push_back(info);
                                            if (pm.isLeader) update.leaderId = pm.characterId;
                                        }
                                    }
                                }
                            }
                        });
                        if (!update.members.empty()) break;
                    }

                    // Broadcast to remaining members
                    uint8_t buf[1024]; ByteWriter bw(buf, sizeof(buf));
                    update.write(bw);
                    for (const auto& mCharId : memberCharIds) {
                        server_.connections().forEach([&](ClientConnection& c) {
                            if (c.character_id == mCharId) {
                                server_.sendTo(c.clientId, Channel::ReliableOrdered,
                                               PacketType::SvPartyUpdate, buf, bw.size());
                            }
                        });
                    }
                }
...
void ServerApp::onPacketReceived(uint16_t clientId, uint8_t type, ByteReader& payload) {
    ...
    switch (type) {
        ...
        case PacketType::CmdParty: {
            processParty(clientId, payload);
            break;
        }
        ...
        case PacketType::CmdAdminSaveContent: {
            auto msg = CmdAdminSaveContentMsg::read(payload);
            if (!validatePayload(payload, clientId, type)) return;
            processAdminSaveContent(clientId, msg);
            break;
        }
        case PacketType::CmdAdminDeleteContent: {
            auto msg = CmdAdminDeleteContentMsg::read(payload);
            if (!validatePayload(payload, clientId, type)) return;
            processAdminDeleteContent(clientId, msg);
            break;
        }
        case PacketType::CmdAdminReloadCache: {
            auto msg = CmdAdminReloadCacheMsg::read(payload);
            if (!validatePayload(payload, clientId, type)) return;
            processAdminReloadCache(clientId, msg);
            break;
        }
        case PacketType::CmdAdminValidate: {
            processAdminValidate(clientId);
            break;
        }
        case PacketType::CmdAdminRequestContentList: {
            auto msg = CmdAdminRequestContentListMsg::read(payload);
            if (!validatePayload(payload, clientId, type)) return;
            processAdminRequestContentList(clientId, msg);
            break;
        }
        default:
