...
private:
    ...
    // Content validation (wired to caches at startup)
    ...
    // Load NPC entities (shops, bankers, teleporters, etc.) from scene JSON into world_
    void loadNPCsFromScenes();
    ...
    // handlers/party_handler.cpp
    void processParty(uint16_t clientId, ByteReader& payload);
    ...
    void processAdminSaveContent(uint16_t clientId, const CmdAdminSaveContentMsg& msg);
    void processAdminDeleteContent(uint16_t clientId, const CmdAdminDeleteContentMsg& msg);
    void processAdminReloadCache(uint16_t clientId, const CmdAdminReloadCacheMsg& msg);
    void processAdminValidate(uint16_t clientId);
    void processAdminRequestContentList(uint16_t clientId, const CmdAdminRequestContentListMsg& msg);
    void sendAdminResult(uint16_t clientId, uint8_t requestType, bool success, const std::string& msg);
    void reloadCacheByType(uint8_t cacheType);
