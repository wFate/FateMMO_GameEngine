...
TEST_CASE("ClientRateLimiter addViolation accumulates weighted violations") {
    ClientRateLimiter limiter;
    double now = 5000.0;

    CHECK(limiter.violations() == 0);

    // Weight=5 adds 5 violations at once
    limiter.addViolation(now, 5);
    CHECK(limiter.violations() == 5);

    // Accumulates
    limiter.addViolation(now, 10);
    CHECK(limiter.violations() == 15);
}

TEST_CASE("ClientRateLimiter addViolation decays after 60 seconds") {
    ClientRateLimiter limiter;
    double now = 6000.0;

    limiter.addViolation(now, 20);
    CHECK(limiter.violations() == 20);

    // 60 seconds later, violations should decay to 0 before adding new ones
    limiter.addViolation(now + 61.0, 1);
    CHECK(limiter.violations() == 1);
}
