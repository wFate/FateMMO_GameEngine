...
#endif
#include <windows.h>
/* windows.h includes the SDK <bcrypt.h> (CNG) which defines BCryptGenRandom.
 * Include our password-hashing header AFTER windows.h to avoid conflicts. */
...
#if defined(_WIN32) || defined(_WIN64)
	// Use modern Windows CNG API for secure random bytes.
	if (BCryptGenRandom(NULL, (unsigned char*)input, RANDBYTES,
